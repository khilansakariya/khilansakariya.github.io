...
const Head = ({ metadata }) => (
  <Helmet>
    ...
    <meta property="og:url" content={metadata.siteUrl || config.siteUrl} />
    ...
    <meta name="twitter:url" content={metadata.siteUrl || config.siteUrl} />
    ...
    {/* <meta itemProp="image" content={`${config.siteUrl}/og.png`} /> */}
      ...
      {`
        {
          ...
          "name": "${config.name}",
          "url": "${config.siteUrl}",
          "jobTitle": "English to Gujarati Translator",
          ...
          "email": "${config.email}",
          "knowsLanguage": ["en-US", "gu"],
          "sameAs": [
            ${config.socialMedia.map(social => `"${social.url}"`).join(',\n            ')}
          ]
        }
      `}
    </script>
